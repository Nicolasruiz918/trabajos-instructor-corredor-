...
SELECT * FROM Vista_Clases_Por_Tipo; 
  ...
  --1 acelera los filtros por Clases 
  CREATE NONCLUSTERED INDEX IX_Clases_id_tipo_entrenamiento
  ON Clases (id_tipo_entrenamiento);
   -- 
     --2 acelera los filtros por Pagos 
  CREATE NONCLUSTERED INDEX IX_Pagos_monto
  ON Pagos (monto);
   -- 

   --3 mejora la busqueda de las entrenadores por nombre 
   CREATE NONCLUSTERED INDEX   IX_Entrenadores_Nombre
   ...
   ON Ejercicios (nombre)
    ...
    --1
    SELECT * FROM Clases WHERE id_tipo_entrenamiento = 1 ;

    --2
    SELECT * FROM Pagos WHERE monto > 25.000 ;
    --3
    SELECT * FROM Entrenadores WHERE nombre like 'Laura Torres';
    --4
